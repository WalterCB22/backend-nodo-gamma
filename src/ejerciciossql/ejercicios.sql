...
/*2- Por otro lado, necesitamos listar los títulos de los episodios junto con el 
nombre y apellido de los actores que trabajan en cada uno de ellos*/
SELECT * FROM episodes;
SELECT * FROM actor_episode;

SELECT title, actor_episode.actor_id, actors.first_name, actors.last_name FROM actor_episode
INNER JOIN episodes
...
/*4- Para nuestro próximo desafío, necesitamos obtener a todos los actores o 
actrices (mostrar nombre y apellido) que han trabajado en cualquier película 
de la saga de La Guerra de las Galaxias*/

SELECT DISTINCT actors.first_name, actors.last_name, movies.title FROM actor_movie AS a
INNER JOIN actors
...
/*5- Crear un listado a partir de la tabla de películas, mostrar un reporte de la cantidad de películas por 
nombre de género.*/

SELECT genre_id, COUNT(genre_id) AS cantidad_peliculas, genres.name FROM movies
INNER JOIN genres 
